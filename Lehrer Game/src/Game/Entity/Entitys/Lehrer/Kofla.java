...
public class Kofla extends Entity{
	private int delay = 0;
	private BufferedImage Lehrer;
	private Handler handler;
	private boolean MoveX = false, MoveY = false;
	public Kofla(int x, int y, ID id, Handler handler) {
		super(x, y, id);
		try { Lehrer = ImageIO.read(getClass().getResourceAsStream("/Lehrer/Kofla.png"));
	...
	private void collision(){
		...
		for(int i = 0; i < handler.entitys.size(); i++){
			...
			if(tempO.getId() == ID.Gous || tempO.getId() == ID.Haidacha || tempO.getId() == ID.Willi|| tempO.getId() == ID.Wurza || tempO.getId() == ID.Kofla){
			...
			if(tempO.getId() == ID.Player){
				if(tempO.BBottom().intersects(BTop())){tempO.setHp(0);}
				if(tempO.BTop().intersects(BBottom())){tempO.setHp(0);}
				if(tempO.BLeft().intersects(BRight())){tempO.setHp(0);}
				if(tempO.BRight().intersects(BLeft())){tempO.setHp(0);}
			}
		}
	}
